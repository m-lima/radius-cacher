    ...
    struct Listener {
      ...
      auto operator()(std::size_t byteCount,
        ...
        switch (action.action) {
          case Action::STORE:
            LOG(logger::DEBUG, "Server::Listener: Storing {:s} with {:s}", *key, *value);
            mCache.set(*(action.key), *(action.value));
            break;
          case Action::REMOVE:
            LOG(logger::DEBUG, "Server::Listener: Removing {:s} with {:s}", *key, *value);
            mCache.remove(*(action.key));
            break;
          case Action::FILTER:
            LOG(logger::DEBUG, "Server::Listener: Filtering {:s}", *value);
            break;
        }
      }
    };
